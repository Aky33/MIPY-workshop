import pygame

class SelectedItemIndicator:
    def __init__(self, inventory, pos=(20, 20), bg_color=(240, 220, 160), border_color=(160, 140, 80)):
        self.inventory = inventory
        self.pos = pos
        self.bg_color = bg_color
        self.border_color = border_color
        self.base_height = 64
        self.icon_size = 40  # Adjust as needed
        self._last_rect = None  # Store last rect for click detection

    def render(self, surf):
        item = self.inventory.get_item(self.inventory.selected)

        font = pygame.font.SysFont("arial", 22, bold=True)
        amount_font = pygame.font.SysFont("arial", 18, bold=True)

        if item:
            # Prepare icon
            icon = item.icon_asset
            icon_rect = icon.get_rect()
            icon_rect.width = self.icon_size
            icon_rect.height = self.icon_size

            # Prepare text
            name_text = font.render(item.name, True, (60, 40, 20))
            amount_text = amount_font.render(f"x{item.amount}", True, (60, 40, 20))

            # Calculate background size
            text_width = max(name_text.get_width(), amount_text.get_width())
            width = 20 + self.icon_size + 12 + text_width + 20  # icon + padding + text + margin
            height = max(self.base_height, 20 + max(self.icon_size, name_text.get_height() + amount_text.get_height() + 8))

            rect = pygame.Rect(self.pos[0], self.pos[1], width, height)
            self._last_rect = rect
            pygame.draw.rect(surf, self.bg_color, rect, border_radius=10)
            pygame.draw.rect(surf, self.border_color, rect, width=3, border_radius=10)

            # Draw icon
            icon_y = self.pos[1] + (height - self.icon_size) // 2
            surf.blit(pygame.transform.smoothscale(icon, (self.icon_size, self.icon_size)), (self.pos[0] + 10, icon_y))

            # Draw name and amount
            text_x = self.pos[0] + 10 + self.icon_size + 12
            surf.blit(name_text, (text_x, self.pos[1] + 14))
            surf.blit(amount_text, (text_x, self.pos[1] + 14 + name_text.get_height() + 4))
        else:
            font = pygame.font.SysFont("arial", 20, bold=True)
            text = font.render("No item selected", True, (100, 80, 40))
            width = text.get_width() + 32
            height = self.base_height
            rect = pygame.Rect(self.pos[0], self.pos[1], width, height)
            self._last_rect = rect
            pygame.draw.rect(surf, self.bg_color, rect, border_radius=10)
            pygame.draw.rect(surf, self.border_color, rect, width=3, border_radius=10)
            surf.blit(text, (self.pos[0] + 16, self.pos[1] + (height - text.get_height()) // 2))

    def get_rect(self):
        # Returns the last drawn rect for click detection
        if self._last_rect is not None:
            return self._last_rect
        # Fallback if not rendered yet
        return pygame.Rect(self.pos[0], self.pos[1], 160, self.base_height)